...
function h(level, text) {
  ...
  return new Paragraph({
    ...
    children: [new TextRun({ text, bold: true, size: sizes[level], font: "Arial" })]
  });
}
...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      ...
      p("Additional PDF capabilities include compression to reduce file size, conversion of PDF pages to images, application of watermarks, optical character recognition for text extraction, addition of headers and footers, redaction of sensitive content, digital signing, thumbnail generation, and general page editing. Each tool is implemented as a separate component that receives the uploaded files as props, keeping the main PDF page clean and maintainable."),
