...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      ...
      p("The project was developed using a contemporary frontend stack consisting of React 18, TypeScript, Vite, Tailwind CSS, and the shadcn/ui component library, all orchestrated under the Lovable Cloud ecosystem with Supabase handling authentication, database, and storage services. The architecture deliberately favors client-side computation for file operations, leveraging powerful JavaScript libraries such as pdf-lib, browser-image-compression, tesseract.js, and the Web Audio API. This design choice enables offline-ready processing, reduces server costs, and ensures that users maintain full control over their data."),
      p("The user interface is organized around a tabbed navigation pattern within each tool category. This design allows users to switch between different operations without losing context or re-uploading their files. For example, in the PDF Tools page, a user can upload a set of PDFs once and then explore merge, split, reorder, compress, and other operations through tabs. This pattern improves efficiency and reduces friction, making the application feel like a cohesive digital workspace rather than a collection of disconnected utilities."),
      ...
      p("Additional capabilities include the Header and Footer tool for adding consistent page branding, the Redact tool for permanently removing sensitive regions, the Sign tool for overlaying digital signatures, the Thumbnails tool for generating page preview images, and the Edit tool for general page-level modifications. Each tool is implemented as a separate component that receives the uploaded files as props, keeping the main PDF page clean and maintainable. The modular design also makes it easy to add, remove, or update individual tools without affecting the rest of the application."),
      p("The PDF page itself manages the shared state of uploaded files, including file objects and page counts. It passes this state down to individual tool panels, which then perform their specific operations. This pattern avoids duplication of file handling logic and ensures that all tools operate on the same source data. When an operation completes, the page calls a shared saveBlob helper that handles both local downloads and optional cloud uploads, promoting consistency across the entire toolset."),
      p("Each PDF operation is implemented with careful attention to error handling and user feedback. For example, when merging files, the code validates that at least two PDFs are present before starting. When splitting, it parses page ranges and validates them against the document length. When reordering, it maintains the visual order in state and maps it back to page indices during export. These small but important details contribute to a reliable and user-friendly experience."),
      ...
      // My Role
      h(1, "State Management and Data Flow"),
      p("PrismDocEditor uses a combination of React Context, local component state, and TanStack Query to manage application state. The AuthContext is the most important global state provider, exposing the current user, authentication status, and helper methods to any component in the tree. This context wraps the application and ensures that authentication state is consistent across routes and components."),
      p("For server state, such as user profile data and file history, the application uses TanStack Query to fetch, cache, and synchronize data with Supabase. This approach provides automatic caching, background refetching, and elegant handling of loading and error states. For purely client-side state, such as uploaded files, processing settings, and UI toggles, the application uses React\u2019s built-in useState and useReducer hooks. This keeps the state close to where it is used and simplifies debugging."),
      p("The data flow for a typical file processing operation follows a clear pattern. First, the user uploads a file through a dropzone, which uses react-dropzone to handle drag-and-drop and file selection. The file is then validated for type and size, decoded or loaded into the appropriate library, and stored in component state. Next, the user configures the desired operation through form inputs, sliders, or buttons. Finally, the operation is executed, the result is converted into a downloadable blob, and the saveBlob helper triggers a local download and optionally uploads the result to Supabase Storage."),

      h(1, "Deployment and Development Workflow"),
      p("The project uses Vite for both development and production builds. During development, Vite provides fast hot module replacement, allowing changes to be reflected in the browser almost instantly. The production build is optimized for performance, with code splitting, tree shaking, and asset optimization handled automatically. The build output can be deployed to any static hosting service, making the application highly portable."),
      p("The codebase is configured with ESLint and TypeScript to enforce code quality and catch errors early. The project includes a Vitest setup for automated testing, encouraging developers to write unit tests for utility functions and integration tests for complex components. While the primary focus during development was on feature delivery, the testing infrastructure provides a foundation for future quality improvements and regression prevention."),
      p("Deployment is handled through the Lovable platform, which provides both preview and published URLs. The preview environment allows developers and stakeholders to test changes before they go live, while the published URL serves the production version to end users. This workflow supports rapid iteration and ensures that the production experience remains stable and polished."),
